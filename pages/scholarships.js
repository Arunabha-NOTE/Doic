...
const Scholarships = () => {
    ...
    const displayedEvents =
        activeTab === "UPCOMING"
            ? universitiesData.filter((event) => event.sno >= 1 && event.sno <= 16)
            : universitiesData.filter((event) => event.sno > 16);
    ...
    return (
        <div>
            <Navbar />
            <div className="bg-gradient-to-r pt-16" >
            ...
            <div
                className= 'mb-4 mx-auto flex max-w-7xl font-bold justify-center pb-2'
            >
                <button
                    className={`mr-4 ${activeTab === "Partnered" ? "border-b-2 border-green-500" : ""}`}
                    onClick={() => setActiveTab("Partnered")}
                >
                    Partnered
                </button>
                <button
                    className={`mr-4 ${activeTab === "Others" ? "border-b-2 border-gray-500" : ""}`}
                    onClick={() => setActiveTab("Others")}
                >
                    Others
                </button>
            </div>
            {displayedEvents.map((event) => (<div className='grid grid-cols-1 md:grid-cols-2 gap-8 max-w-7xl mx-auto'>
                {scholarshipInfo.map((info, index) => (
